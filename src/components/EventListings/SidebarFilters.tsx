...
const SidebarFilters: React.FC = () => {
  ...
  const handleClear = (filterName: string) => {
    ...
    // Add clearing logic for other filters as needed
  };
  ...
  const FilterTrigger: React.FC<{ title: string }> = ({ title }) => (
    <div className="flex items-center justify-between w-full">
      ...
      <Button
        ...
        onClick={(e) => {
          e.stopPropagation(); // Prevent accordion toggle when clearing
          handleClear(title);
        }}
      >
        Clear
  ...
  return (
    <div className="w-64 bg-card flex flex-col h-full border-r">
      <h2 className="text-2xl font-bold px-6 pt-6 pb-2">Filters</h2>
      <div className="flex-grow overflow-y-auto pr-2 pl-6">
